...
from openeo_pg_parser_networkx.process_registry import ProcessRegistry, Process
...
def test_execute_returns_result_and_workflow(process_graph_path):
    ...
    assert isinstance(workflow, Workflow), "Workflow should be a yprov4wfs.Workflow instance"
    ...
    for task in workflow._tasks:
        ...
        assert isinstance(task, Task), f"Each task should be a Task instance but got {type(task)}"
    ...
    for data in workflow._data:
        assert isinstance(data, Data), f"Each data node should be a Data instance but got {type(data)}"
