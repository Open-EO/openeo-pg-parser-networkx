...
from eodc_pg_parser.pg_schema import ProcessArgument, ProcessGraph, ProcessNode, ResultReference
...
def test_data_types_explicitly():
    ...
    nested_process_graph = OpenEOProcessGraph._unflatten_raw_process_graph(flat_process_graph)
    ...
    assert isinstance(parsed_process_graph.process_graph["root"].arguments["model"], ProcessArgument)
    assert isinstance(parsed_process_graph.process_graph["root"].arguments["model"].__root__, ResultReference)
    assert isinstance(parsed_process_graph.process_graph["root"].arguments["model"].__root__.node, ProcessNode)
